...
class ResNetBasicblock(nn.Module):
  ...
  """
    ...
    def forward(self, x, nolast=False):
        ...
        # print("size:", t1*t2*t3)
        return F.relu(residual + basicblock, inplace=True), self.output(residual + basicblock, nolast)
    ...
    def flip_out(self, x, nolast=False):
        return F.relu(x, inplace=True), self.output(x,nolast)
...
class CifarResNet(nn.Module):
  ...
  """
    ...
    """
        ...
        self.inplanes = 16
        self.branch_linearshape = [65536, 65536, 65536, 65536, 65536, 16384, 16384, 16384, 16384, 16384, 4096, 4096, 4096, 4096, 4096, 9216, 1024]
        self.b_index = 0
        ...
        #print("64 * block.expansion, num_classes:", 64 * block.expansion, num_classes)
        self.classifier = quan_Linear(256, num_classes)
  ...
  """
    ...
    def forward(self, x, nolast=True):
        ...
        for g in range(1, 4):
          layer_num = len(getattr(self, 'group{}'.format(g)))
          for i in range(layer_num):
            x, branch_out = getattr(self, 'group{}'.format(g))[i](x,nolast=True)
            output_branch.append(branch_out)
...
def resnet20_quan_ti(num_classes=200):
...
def resnet32_quan_ti(num_classes=200):
...
def resnet32_quan_ti1(num_classes=200):
    """Constructs a ResNet-32 model for CIFAR-10 (by default)
  Args:
    num_classes (uint): number of classes
  """
...
def resnet44_quan_ti(num_classes=200):
...
def resnet56_quan_ti(num_classes=200):
...
def resnet110_quan_ti(num_classes=200):
