...
# define my own func
def vote_for_predict(output_branch, target, topk):
    with torch.no_grad():
        batch_size = target.size(0)
        l_p = []
        _, pred_final = output_branch[0].topk(1, 1, True, True)
        for i in range(1, len(output_branch)-1):
            _, pred = output_branch[i].topk(1, 1, True, True)
            pred_final = torch.cat([pred_final, pred], 1)
            #sl_p.append(pred.t())
        for i in range(pred_final.shape[0]):
            ans = torch.argmax(pred_final[i].bincount())
            l_p.append(ans)
            #print(ans)
        l_p = torch.tensor(l_p).unsqueeze(axis=0).cpu()

        target = target.cpu()
        
        correct = l_p.eq(target.view(1, -1).expand_as(l_p))
        res = []
        for k in topk:
